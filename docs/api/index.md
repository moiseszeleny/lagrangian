# API Reference

Auto-generated from docstrings via `sphinx.ext.autosummary`. Each module
page lists its public classes and functions with full signatures; cross-refs
(`{func}`, `{class}`) used throughout the {doc}`Algorithms Manual
</manual/pipeline>` link here.

```{eval-rst}
.. autosummary::
   :toctree: generated
   :recursive:

   feynlag.parameters
   feynlag.fields
   feynlag.groups.base
   feynlag.groups.gauge
   feynlag.groups.discrete
   feynlag.lagrangian
   feynlag.operators
   feynlag.invariance
   feynlag.dirac
   feynlag.vacuum.ewsb
   feynlag.vacuum.tadpoles
   feynlag.vacuum.masses
   feynlag.vacuum.diagonalize
   feynlag.vertices.extract
   feynlag.vertices.bilinear
   feynlag.vertices.vertex
   feynlag.vertices.yangmills
   feynlag.export.latex
   feynlag.export.ufo.writer
   feynlag.export.ufo.vvvv
   feynlag.export.ufo.lorentz_map
   feynlag.verify.checks
```
